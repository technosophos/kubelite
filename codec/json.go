...
func (d jsonDecoder) All() ([]*Manifest, error) {
	...
	if err := json.Unmarshal(d.data, &phony); err == nil {
		...
		return []*Manifest{&Manifest{data: d.data, dec: jdec}}, nil
	}
	...
	if len(lines) < 2 {
		return []*Manifest{}, errors.New("Data is neither JSON nor JSONL. (linecount)")
	}

	println(string(lines[0]))
	if err := json.Unmarshal(lines[0], &phony); err != nil {
		// Whoops.. failed again.
		return []*Manifest{}, errors.New("Data is neither JSON nor JSONL: " + err.Error())
	}

	buf := make([]*Manifest, len(lines))
	for i, l := range lines {
		buf[i] = &Manifest{data: l, dec: jdec}
	}
	return buf, nil
}
